...
function enableButton() {
    ...
    var order = document.getElementById("order");//קליטת הכפתור

    var regexName = /^[א-תa-zA-Z\s]+$/; //משתנה שמגדיר מה מותר שיהיה במקרה הזה לשם רק תווים בעברית ואנגלית ורווח
    var regexAdress = /^[א-תa-zA-Z0-9\s]+$/;//משתנה שמגדיר מה מותר שיהיה במקרה הזה תווים בעברית אנגלית ומספרים

    
    if (name.length > 1 && adress.length > 1 && shipping && plent.length > 0) { //בודק שהמשתמש הזין הכל לפי התנאים הנדרשים
        if (!regexName.test(name) || !regexAdress.test(adress)) { //בודק האם המשתמש הזין רק אותיות בשם
            alert("אחד או יותר מהשדות שהקשת לא תקין"); //מראה התראה במידה והקיש מקש לא תקין
        }
        else {
...
function btn() {
    ...
    var remarks = document.getElementById("message").value;
...
function selectOptionsAndUpdateImages(options) { //פוקציה שקולטת משתנה מערך
